...
let cout_ticket = document.getElementById(
    "inputprixticket"
) as HTMLInputElement;
...
export function eventrendu() {
    cout_ticket.oninput = function (e: Event) {
        e.preventDefault();
        definirRendu();
    };
    paiement.oninput = function (e: Event) {
        e.preventDefault();
        definirRendu();
    };
}
...
function definirRendu() {
    ...
    if (!isNaN(ticket) && !isNaN(paiement_cl)) {
        if (paiement_cl >= ticket) {
            ...
            // Razles inputs
            let inputcinquante = document.getElementById(
                "cinquanteeuro"
            ) as HTMLInputElement;
            let inputvingt = document.getElementById(
                "vingteuro"
            ) as HTMLInputElement;
            let inputdix = document.getElementById(
                "dixeuro"
            ) as HTMLInputElement;
            let inputcinq = document.getElementById(
                "cinqeuro"
            ) as HTMLInputElement;
            let inputdeux = document.getElementById(
                "deuxeuro"
            ) as HTMLInputElement;
        ...
        erreurRendu(
            `Il y a un problème avec le prix du ticket (${
                isNaN(ticket) ? "Non défini" : "Autre : " + ticket
            }) ou le paiement du client (${
                isNaN(paiement_cl) ? "Non défini" : "Autre : " + paiement_cl
            })`,
            "bg-danger-subtle"
        );
    }
